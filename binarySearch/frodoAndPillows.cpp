...
void solve(){
    ...
    int leftRange = k, rightRange = n - k; 
    auto check = [&](int x) -> bool {
        int leftPillows = 0, rightPillows = 0;
        if(leftRange >= x){
            leftPillows = (leftRange - x) + (x * ( x + 1) / 2);
        }else {
            leftPillows = leftRange * ( 2 * x - leftRange + 1) / 2;
        }

        if(rightRange >= x){
            rightPillows = (rightRange - x + 1) + ( x * ( x - 1) / 2);
        }else {
            rightPillows = rightRange * ( 2 * x - rightRange - 1) / 2;
        }
