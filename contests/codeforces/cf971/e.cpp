...
void solve(){
    ...
    int n=nxt(), k=nxt();
    auto sum = [&](int m, int l) -> int {
        return (l - m + 1) * (m + l)/2;
    };
    int left = k, right = k + n - 1;
    while(left < right)
    {
        int mid = left + (right - left)/2;
        if(sum(k, mid) < sum(mid, k+n-1)) left = mid + 1;
        else right = mid;
    }
    cout << min(abs(sum(k, left) - sum(left+1, k+n-1)), abs(sum(k, left-1) - sum(left, k+n -1))) << endl;
}
