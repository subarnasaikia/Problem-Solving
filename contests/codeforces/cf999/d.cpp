...
void solve(){
    ...
    auto isEqual = [&](map<int, int> a, map<int, int> b){
        if(a.size() != b.size()) return false;
        for(auto i : a){
            if(b.find(i.fi) == b.end()) return false;
            if(b[i.fi] != i.se) return false;
        }
        return true;
    };
    if( n == m ){
    ...
    int sumA = 0, sumB = 0;
    for(auto i : a) sumA += i.fi * i.se;
    for(auto i : b) sumB += i.fi * i.se;
    if(sumA != sumB){
        cout << "NO\n";
        return;
    }
