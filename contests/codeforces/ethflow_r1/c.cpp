...
void solve(){
    ...
    vll ans;
    int sum = accumulate(all(arrs[0]), 0ll);
    ans.push_back(sum);
    rep(i, n)
    {
        if(i == 0) continue;
        for(int j = 1; j<arrs[i-1].size(); j++)
        {
            arrs[i].push_back(arrs[i-1][j] - arrs[i-1][j-1]);
        }
        int sum1 = accumulate(all(arrs[i]), 0ll);
        if(sum1 < 0) sum1 *= -1ll;
        ans.push_back(sum1);
    }
    // for(auto ansv: ans)
