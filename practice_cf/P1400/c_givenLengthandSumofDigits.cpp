...
void solve(){
    ...
    if(str_with_9.size() > m || (str_with_9.size() == m && s != 0))
    {
        return void(cout << "-1 -1" << endl);
    }
    max_ans += str_with_9, min_ans += str_with_9;
    if(str_with_9.size() < m)
    {
        int rest_size = m - (int)(str_with_9.size());
        max_ans += to_string(s);
        rep(i, rest_size-1)
        {
            max_ans += '0';
