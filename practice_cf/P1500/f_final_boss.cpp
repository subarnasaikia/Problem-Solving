...
void solve(){
    ...
    auto check = [&](int r) -> bool {
        if(r == 0) return false;
        r--;
        unsigned long long total_attack = sumA;
        rep(i, n)
        {
            total_attack += 1ll*(r/c[i])*a[i];
        }
        if(total_attack >= h) return true;
        return false;
    };

    int start = 1, end = h*(maxv(c));
    while(start <= end)
    {
    ...
    if(check(end)) cout << end << endl;
    else cout << end + 1 << endl;
}
